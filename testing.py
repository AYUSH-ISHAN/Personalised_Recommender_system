...
a = 'The product recommendation for a particular user on basis of his preference'
print(a)
print('Product ID    Their rating prediction for a given user')
...
#   print(config)
  ...
  for epoch in range(start_epoch, alg_parametes.epoch + 1):
    ...
    #   print("train : ", acc, loss)
      ...
      prediction_list = []
      for j,k in zip(pred, train_set_X[start:end]):
        prediction_list.append([k[1].detach(),j.detach()])
    #   print(prediction_list)
      prediction_list.sort(key=lambda x:x[1])
    #   print(sorted(prediction_list))
      list = prediction_list.reverse()
