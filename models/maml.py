...
def make(clf_name, d_model, d_hidden, n_layers, n_head, d_k, d_v, n_position):
  """
  ...
  """
  enc = encoders.make(d_model, d_hidden, n_layers, n_head, d_k, d_v, n_position)
  # clf_args['in_dim'] = enc.get_out_dim()
  clf = logistic.LogisticClassifier()#classifiers.make(clf_name)
  model = MAML(enc, clf)
  return model
...
class MAML(Module):
  ...
  def _inner_forward(self, x, params, episode):
    ...
    # print(feat.shape)
    logits = self.classifier.forward(feat, get_child_dict(params, 'classifier'))
    # print("logits : ", logits)
    return logits
  ...
  def forward(self, x_shot, x_query, y_shot, inner_args, meta_train):
    ...
    # print("named params : ", params)
    # for name in list(params.keys()):
    #   if not params[name].requires_grad or \
    #     any(s in name for s in inner_args.frozen + 'temp'):
    #     params.pop(name)
    # print("named params : ", params.items())
    logits = []
    # print(x_shot)
    for ep in range(x_shot.shape[0]):
      # inner-loop training
      self.train()
      if not meta_train:
        for m in self.modules():
